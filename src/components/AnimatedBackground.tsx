...
const generateShapes = (width: number, height: number, density: number, minSize: number, maxSize: number, colors: string[]): Shape[] => {
...
const ShapeComponent: React.FC<{
  ...
  const y = useTransform(parallaxY, [0, 1], [0, disableParallax ? 0 : -shape.speed * 30]);
  const renderShape = () => {
    const shapeProps = {
      fill: shape.color,
    ...
    switch (shape.type) {
      case "circle":
        return <circle cx={shape.size / 2} cy={shape.size / 2} r={shape.size / 2} {...shapeProps} />;
      case "square":
        return <rect width={shape.size} height={shape.size} {...shapeProps} />;
      case "triangle":
  ...
  return <motion.div style={{
    ...
    willChange: "transform"
      ...
      <svg width={shape.size} height={shape.size} style={{
      display: "block"
    }}>
        {renderShape()}
      </svg>
...
const AnimatedBackground: React.FC<AnimatedBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const shapesData = generateShapes(dimensions.width, dimensions.height, variantConfig.density || density, minSize, maxSize, variantConfig.colors || colors);
  ...
  return <div className={`relative min-h-screen overflow-hidden ${variantConfig.background} ${className}`} ref={containerRef}>
      <div className="relative w-full h-auto overflow-hidden\n">
        {shapes.map(shape => <ShapeComponent key={shape.id} shape={shape} parallaxY={scrollProgress} disableParallax={shouldDisableParallax} />)}
      </div>
